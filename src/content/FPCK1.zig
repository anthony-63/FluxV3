...
// format spec
// section seperated by 66 70 63 6B 31 01 03 01 (fpck1)
...
const L0SEP = [8]u8{ 0x66, 0x70, 0x63, 0x31, 0x01, 0x03, 0x01, 0xf3 };
...
fn createFpck1Tree(dir: std.fs.Dir, allocator: std.mem.Allocator) !std.ArrayList(u8) {
    ...
    std.debug.print("[FPCK1] storing file tree\n", .{});
    ...
    while (try walker.next()) |entry| {
        ...
        var data_len_buffer: [8]u8 = [8]u8{ 0, 0, 0, 0, 0, 0, 0, 0 };
...
pub fn pack(to_pack: std.fs.Dir, output: std.fs.File, allocator: std.mem.Allocator) !void {
    ...
    std.debug.print("total archive size before compression: {d}kb\n", .{@as(f64, @floatFromInt(tree.items.len)) / 1000.0});
    var stream = std.io.fixedBufferStream(tree.items);
...
pub fn unpack(file: std.fs.File, output: std.fs.Dir, allocator: std.mem.Allocator) !void {
    ...
    var sections_iter = std.mem.split(u8, data, &L0SEP);

    const tree = sections_iter.next().?;
    const files = sections_iter.next().?;
